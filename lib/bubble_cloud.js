...
export default function(elem, partyMap) {
    ...
    const setData = function(rawData) {
        ...
        dataMax = d3Max(rawData, d => +d['share']);
    ...
    const sizeGraph = function() {
        ...
        if(simulation) {
            simulation.force('x').x(center.x);
            simulation.force('y').y(center.y);
    ...
    const chart = function() {
        ...
        simulation = d3ForceSimulation()
            .force('collide', d3ForceCollide(d => d.radius + 1).iterations(16))
            .force('x', d3ForceX().strength(forceStrength).x(center.x))
            .force('y', d3ForceY().strength(forceStrength*3).y(center.y))
            .on('tick', ticked);
