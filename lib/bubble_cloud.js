...
export default function(elem, partyMap) {
    ...
    const updateVisualisation = function(nodes) {
        ...
        bubbles = bubbles.enter()
            ...
            .on('click', function() {
                ...
                ev.initCustomEvent('word-click', false, false, d3Select(this).datum());
                sel.node().dispatchEvent(ev);
            })
            .merge(bubbles)
    ...
    const createPartySim = function(word) {
        ...
        partyGroup.selectAll('.partyBubble')
            ...
            .on('click', function() {
                const ev = document.createEvent("CustomEvent");
                ev.initCustomEvent('party-click', false, false, d3Select(this).datum());
                sel.node().dispatchEvent(ev);
            });
    ...
    const sizeGraph = function() {
        ...
        if(height < 500 && height < window.innerHeight) {
