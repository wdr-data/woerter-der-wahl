...
export default function() {
    ...
    function createNodes(rawData, options) {
        ...
        const valKey = options.valKey || 'count';
    ...
    function ticked() {
        ...
        if(selectedNode !== null) {
            const currentNode = d3.select('#bubble-'+selectedNode).datum();
    ...
    const scaleBubble = function(id, enabled) {
        const bubble = d3.select('#bubble-'+id);
        const word = d3.select('#word-'+id);
    ...
    const selectNode = function(node) {
        ...
        if(!selection) {
            ...
            createPartySim(dataById[id].party_counts);
        }
    ...
    const setupPartySim = function() {
        partySim = d3.forceSimulation()
            ...
            .force('x', d3.forceX().strength(d => d.id == 'ghost' ? 0.3 : 0.03).x(center.x))
            .force('y', d3.forceY().strength(d => d.id == 'ghost' ? 0.3 : 0.03).y(center.y))
            .on('tick', partyTicked);

        partySim.stop();
    };

    const createPartySim = function(parties) {
        const data = Object.keys(parties).map(k => {
            return {
                id: k,
                name: k,
                count: parties[k]
    ...
    return function chart(selector, rawData) {
        ...
        const bubblesE = bubbles.enter().append('circle')
            ...
            .attr('r', 0)
            .on('click', function() {
                selectNode(d3.select(this).datum());
            });
